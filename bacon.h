...
void AGBWriteROMSequentialBytes(uint32_t addr, const std::vector<uint8_t> &data, bool hwaddr = false, bool reset = true, transfer_func transfer = transfer);
void AGBWriteROMWithAddress(const std::vector<std::pair<uint32_t, uint16_t>> &commands, bool hwaddr = false, bool reset = true, transfer_func transfer = transfer);
